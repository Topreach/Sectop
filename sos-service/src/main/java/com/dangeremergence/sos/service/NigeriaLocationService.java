...
/**
 * Nigeria reverse geocoding service for the SOS microservice.
 * Resolves GPS coordinates to State and Local Government Area (LGA) names.
 * <p>
 * Simplified version — only includes the resolution logic needed for SOS alerts.
 */
@Service
@Slf4j
public class NigeriaLocationService {
    ...
    public void init() {
        ...
        states.add(new StateBoundary("Kwara", 7.5, 9.0, 3.0, 5.5, Arrays.asList(
        ...
        states.add(new StateBoundary("Ogun", 6.5, 7.8, 2.5, 4.5, Arrays.asList(
        ...
        states.add(new StateBoundary("Rivers", 4.2, 5.5, 6.3, 7.5, Arrays.asList(
        ...
        states.add(new StateBoundary("Yobe", 10.5, 13.5, 9.5, 12.5, Arrays.asList(
        ...
        log.info("NigeriaLocationService initialized with {} states", states.size());
    }

    /**
     * Resolve GPS coordinates to [State, LGA].
     */
    public String[] resolve(Double lat, Double lng) {
        ...
        for (StateBoundary state : states) {
            if (state.contains(lat, lng)) {
                String lga = resolveLga(state, lat, lng);
                return new String[]{state.name, lga};
            }
        }

        // Fallback: find nearest state by centroid distance
        StateBoundary nearest = null;
        double minDist = Double.MAX_VALUE;
        for (StateBoundary state : states) {
            double d = distance(lat, lng, state.centerLat(), state.centerLng());
            if (d < minDist) {
                minDist = d;
                nearest = state;
            }
        ...
        if (nearest != null) {
            return new String[]{nearest.name, nearest.lgas.get(0)};
        }

        return new String[]{"Unknown", "Unknown"};
    }

    private String resolveLga(StateBoundary state, double lat, double lng) {
        if (state.lgas.isEmpty()) return "Unknown";
        // Simple hash-based LGA assignment for even distribution
        int index = Math.abs((int) (lat * 1000 + lng * 1000)) % state.lgas.size();
        return state.lgas.get(index);
    }
